...
class AddInstitutionProfileProvider extends ChangeNotifier {
  ...
  addDataToFirebase() async {
    ...
    if (ischeck) {
      try {
        instituteProfileModel.createat = DateTime.now();
  ...
  chackValidation() {
    ...
    else if (instituteProfileModel.firstRegistrationDateWithHera == null) {
      Get.snackbar(
          'Alert', 'Please enter select first registration date with hera',
          backgroundColor: Colors.teal,
          snackPosition: SnackPosition.BOTTOM,
          maxWidth: 400);
      return false;
